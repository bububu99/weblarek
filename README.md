...
`_errors: FormErrors` - хранит в себе текущие ошибки в заполнении формы покупателем.
...
`   phone?: string;`
`   address?: string`
` }): void)` - частично или полностью сохраняет данные покупателя в модели.
`get buyer(): IByuer` - получает все данные покупателя.
`clear(): void` - очищает данные покупателя.
`validate(): FormErrors` - проверяет все поля на заполненность. Возвращает объект с текстами ошибок для пустых полей.
`get formErrors(): FormErrors` - получает результат последней валидации.
